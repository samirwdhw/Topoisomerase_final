...
#define FILE_NAME "timeVsEnzymes_expo.dat"		//To see where to output data
...
void calcProb(int pos){	//To update the probability
	...
	if( pos <= N/2){
		...
		f_each = FORCE*list.cats_left*exp((float)pos/NORMAL)/(n_cats*list.left_sum);

	}

	else{
			
		f_each = FORCE*list.cats_right*exp((float)(N - pos)/NORMAL)/(n_cats*(list.right_sum));
	}
